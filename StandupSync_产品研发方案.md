...
> 基于需求文档 V1.0 与功能列表（71 项功能）制定，适用于 PySide6 桌面 + Java Spring Boot + MySQL 架构。
...
│   │   ├── config/             # Security、WebSocket、CORS 配置
│   │   ├── model/              # JPA Entity
│   │   │   ├── User.java
│   │   │   ├── Team.java
│   │   │   ├── TeamMember.java
...
│   │   ├── dto/                # 请求/响应 DTO
│   │   ├── controller/         # REST Controller（按模块拆分）
│   │   │   ├── AuthController.java
│   │   │   ├── UserController.java
│   │   │   ├── TeamController.java
...
│   ├── main.py                 # 入口，启动 Spring Boot 子进程 + 主窗口
...
### 2.1 后端骨架（Java Spring Boot）

| 任务 | 内容 |
|:--:|------|
| P0-1 | Spring Boot 项目初始化（Spring Initializr：Spring Web + Spring Data JPA + Spring WebSocket + Spring Security + MySQL Driver） |
| P0-2 | `application.yml` 配置（MySQL 连接、JWT 密钥、CORS） |
| P0-3 | JPA Entity 创建（User, Team, TeamMember, Meeting, MeetingParticipant, MeetingSpeech, ActionItem），`spring.jpa.hibernate.ddl-auto=update` 自动建表 |
| P0-4 | Spring Security + JWT 过滤器（登录返回 Token，请求校验 Token） |
| P0-5 | 统一响应格式 `ApiResponse<T>`（code, message, data） |
| P0-6 | CORS 配置（允许桌面端本地调用） |
...
| P0-12 | 后端自动启动：桌面 `main.py` 启动时 `subprocess.Popen` 拉起 `java -jar backend.jar` |
...
```
AIService (backend/.../service/AIService.java)
├── analyzeMeeting(speeches, userAiConfig) → 三步流水线
│   Step 1: 发言人识别（粘贴场景）
│   Step 2: 逐人结构化
│   Step 3: 汇总 + Action Item
│
├── callLLM(provider, model, apiKey, messages) → 通用 LLM 调用
│   使用 OkHttp 发送 HTTP POST，支持 OpenAI 兼容 API
│   支持: 豆包 / 通义千问 / OpenAI 等
│
...
*方案版本：V2.0 | 制定日期：2026-06-28 | 技术栈：PySide6 + Java Spring Boot + MySQL*
